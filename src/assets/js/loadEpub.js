...
url = `/assets/books/${bookTitle.replace(/ /g, "_")}.epub`;
...
var currentSectionIndex = JSON.parse(localStorage.getItem("currentPosition"));

// Load the opf
var book = ePub(url || "/assets/books/Pride_and_Prejudic.epub");
...
rendition.on("rendered", function (section) {
    ...
    if (current) {
        ...
        for (var i = 0; i < $options.length; ++i) {
            let selected =
                            $options[i].getAttribute("ref") === current.href;
            if (selected) {
